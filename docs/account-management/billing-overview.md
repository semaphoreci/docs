...
Each plan comes with a monthly subscription cost and includes access to all the basic resources you need. Any additional spending, such as extra machine time, storage and egress, seats, or add-ons, will be calculated daily and added to your monthly bill.
...
!!! info "Only _running_ time is billed"
    Please note that time spent in the _queue_ state due to concurrency limits or [pipeline queues](/essentials/pipeline-queues/) is not counted towards the machine time spent.
...
#### How to monitor and optimize machine spending
...
- **Track Performance**: Monitor your pipeline performance through the pipeline insights tab and regularly review your spending trends on the billing page.
...
- **Egress** - Network traffic generated by downloading artifacts within jobs or to an external location. This is charged in GB, where one "unit" represents 1 GB of transferred data.
...
#### How to monitor and optimize storage spending
...
#### How to monitor and optimize Egress spending
...
#### How to monitor and optimize Seat spending
...
- Properly configure your project's [workflow triggers](/essentials/project-workflow-trigger-options/) to ensure you are only building the necessary builds (e.g., skip builds for forked PRs).
...
- **Dedicated Cache Server**: Provides 400 GB of storage with 10 Gb/s network, and costs $400 per month. Ideal for workflows that require high sequential I/O, network performance, and disk throughput.
- **Dedicated Container Registry**: Offers 400 GB of storage with 10 Gb/s network, and costs the same amount as the dedicated cache server. Improves the speed of your Docker-based builds by storing your images in the same infrastructure as your jobs run on.
