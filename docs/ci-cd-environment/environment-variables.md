...
The `SEMAPHORE_WORKFLOW__RERUN` environment variable remains the same during
a pipeline run and is available in all the blocks of a pipeline as well as in
all promoted and auto promoted pipelines of the same workflow.
...
The `SEMAPHORE_WORKFLOW__TRIGGERED_BY_HOOK` environment variable remains the same
