...
By default, Semaphore will create a workflow for push to every branch. You can limit
...
!!! info "Semaphore uses a MERGE commit to run a workflow."
   Instead of a HEAD commit pushed to the pull request, Semaphore uses the MERGE commit between the source and upstream branch.
