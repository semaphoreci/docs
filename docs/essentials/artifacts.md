...
- **Storage** - The amount of data stored in your artifact buckets. This is expressed in per GB per month. Where each unit is 1GB of artifacts stored for one month.
- **Traffic** - The network traffic generated by downloading artifacts either inside jobs or to an external location. Expressed in GB where each unit is 1GB of data transferred. 
