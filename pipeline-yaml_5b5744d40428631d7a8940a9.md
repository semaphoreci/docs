...
The `dependencies` property defines a list of `block` item names on a per
...
An empty list of `dependencies` is defined as follows:
...
The `auto\_promote\_on` property
...
The contents of `p1.yml` are the following:
