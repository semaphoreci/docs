...
The `martix` property allows you to define one or more environment variables
with multiple values. After that, it creates multiple jobs by calculating the
Cartesian product of all the sets of user defined values.

So, the final outcome of the `matrix` property is concurrent jobs with the
commands that are defined in the respective `jobs` property but different
environment variables that are defined in the `matrix` property.
...
This will create six jobs because there are 6 elements in the Cartesian product
of the environment variables defined in the `matrix` property.

Based on the user defined name of the `job` property that used `matrix`, the
names of the 6 jobs will be as follows:
