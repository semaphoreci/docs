...
limit for the entire pipeline as well as for individual tasks.

The `execution_time_limit` property can be used in two ways. First, in a global
way where you define the execution time limit for the entire pipeline and
second, in a local way where you define the execution time limit for a single
block. In the latter case you can have multiple `execution_time_limit`
properties in multiple blocks.
...
and minutes, respectively. You are allowed to use a combination of these two
properties.

The types of the values of both `hours` and `minutes` properties are integers
and their values should be greater than or equal to 1.
...
a pipeline only has local `execution_time_limit` properties, the global value
of `execution_time_limit` will still be **1 hour**.

### An example with a global execution\_time\_limit

The following pipeline YAML file uses a global `execution_time_limit` property:
...
### An example with a local execution\_time\_limit

The following pipeline YAML file uses two local `execution_time_limit` properties:
...
### ### An example with both global and local execution\_time\_limit

The following pipeline YAML file uses a combination of a global
`execution_time_limit` property and a single *local* `execution_time_limit`
property:
...
In this case the `Creating Docker Image` task should finish in less than 15
minutes whereas the `Building executable` task should finish in less than 5
hours, which is the value of the global `execution_time_limit` property.
...
file that contains the commands of a job instead of listing them in a
`commands` list.
...
The `promotions` property is used for *Promoting* (manually triggering) one or
more pipelines using one or more pipeline YAML files, respectively. A pipeline
YAML file can have a single `promotions` block or no `promotions` block at all.
