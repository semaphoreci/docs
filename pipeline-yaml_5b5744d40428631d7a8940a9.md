...
A `secrets` bucket is a place where you define and store environment variables
that you do not want to make public in a Pipeline YAML file or in a text file
that is stored in the GitHub repository of your project. A `secrets` bucket is
defined using a different YAML grammar and imported using the `sem` utility.
...
buckets are the same, then the shared environment variables will have the value
that was found in the `secrets` bucket that was imported last.
...
`secrets` bucket that you want to import. Please note that the specified
