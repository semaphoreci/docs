...
You can store one or more files in a `secret`, which means that you should be
able to access these files from a pipeline YAML file.

You do not need any extra work for using a file that you stored in a `secret`
apart from including the name of the secret in a `secrets` list. After that
the only other requirement is that you should remember the name of the file,
which is the value you put at the `path` property of the `secret`.

All files in secrets are store in the home directory of the user of the Virtual
Machine (VM), which is `/home/semaphore`.
