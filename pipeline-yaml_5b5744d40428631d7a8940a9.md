...
create. The properties of the unofficial preface part are `version`, `name`,
...
displayed in the Semaphore 2.0 GUI.
...
Please notice that a project *will fail* if a command in a `prologue` block
...
A `secrets` bucket is a place where you define environment variables that you
do not want to make public in a Pipeline YAML file or in a text file that is
stored in the GitHub repository of your project. A `secrets` bucket is defined
using a different YAML grammar and imported using the `sem` utility.
...
the `secrets` bucket that you want to import. Please note that the specified
...
You can trigger a `switch` child at any point, even while the pipeline that
owns that `switch` child is still running.
...
YAML file as defined by this document.
