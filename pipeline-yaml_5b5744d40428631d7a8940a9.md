...
The `auto_promote_on` property is a list of items that can have any combination
of the three supported properties: `result`, which is mandatory, `branch`, which
is optional and `result_reason`, which is also optional.
...
* `stopped`: the pipeline was stopped either by user or by the system
* `canceled`: the pipeline was cancelled either by user or by the system.
    The difference between `canceled` and `stopped` is that you can only cancel
    a pipeline that is not already running.
...
In order for a `branch` value to match the `master` branch only and not branch
...
In order for a `branch` value to match everything that begins with `dev` you
should try something like `^dev`.

#### result_reason

The value of the `result_reason` property is a string and signifies the reason
that the `result` property value happened.
...
* `deleted`: terminated because branch was deleted while pipeline was running
* `internal`: terminated for internal reasons
* `user`: stopped on user request
...
`failed` and the `result_reason` is `malformed` and the GitHub branch name
must contain the `v2` sequence of characters following by at least one more
