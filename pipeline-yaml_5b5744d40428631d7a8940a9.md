...
with multiple values. After that, it creates multiple parallel jobs by
calculating the Cartesian product of all sets of values.

So, the final outcome of the `matrix` property is parallel jobs with the
commands that are defined in the respective `jobs` property joined with all
possible combinations of the environment variables defined in the `matrix`
property.
...
In this example the values of the `matrix` property expand to 6 parallel jobs
as there are 2 * 3 = 6 combinations of the provided environment variables:
