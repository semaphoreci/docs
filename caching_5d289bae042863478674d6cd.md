...
* **Node.js** (npm, yarn and nvm) - default cache path: `node_modules` if `package-lock.json` is present or `node_modules`, `/$HOME/.cache/yarn` if `yarn.lock` exists in the repository and `$HOME/.nvm` if `.nvmrc` is present in the repository.
