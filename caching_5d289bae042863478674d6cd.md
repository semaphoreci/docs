Caching is the one of the best ways to speed up the execution of jobs in a
CI/CD pipeline. It is particularly useful when you are developing software that
depends on other libraries which are fetched via the internet.

On Semaphore, every project gets a dedicated cache storage optimized for CI/CD
that is shared between jobs. The `cache` command line tool is the primary way
to access the cache storage in your jobs.

- [Architectural principles of the Cache Storage system]()
- [Limitations of the Cache System]()

Commonly asked questions:

- [What is the difference between Cache and Artifacts?]()
- [What do I need to access the Cache in custom built Docker images?]()
