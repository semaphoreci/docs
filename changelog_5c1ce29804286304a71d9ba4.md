...
- You can now change the initial pipeline for your project, 
  from `.semaphore/semaphore.yml` to a custom path. This allows 
  creating multiple projects that are connected to the same repository
  on Github with different execution steps. The new option
  is available on the project settings page, or via the latest release 
  (v0.15.0) of the CLI. 
