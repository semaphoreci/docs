...
`my-example-secrets`. The desired file is the following:
...
in order to paste it in a while.

Execute the next command in order to edit the contents of the
`my-example-secrets` secret using your favorite UNIX editor:

    sem edit secrets my-example-secrets

Now, you can include that file along with the output of `base64 file.txt` in
the `my-example-secrets` secret by adding the following text in it:

    files:
    - path: file.txt
      content: <base64 output>
...
of the Virtual Machine (VM) and it will be automatically *decoded*.
...
`sem get secrets` command.

Then, you will have to populate that `secret` by editing its contents with the
help of the next command:
...
want. Later on in this document you will learn more ways of editing the
contents of existing `secret`.
...
current organization:
    ...
    sem get secrets | grep -v NAME | awk '{print $1}' | xargs -n1 sem describe secrets
...
Alternatively, you can edit the contents of a `secret` directly:

    sem edit secrets my-secrets

There is also a technique that can be applied when you do not have the original
YAML file and requires the execution of the following three commands:
