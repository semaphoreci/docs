...
same `secrets` bucket as the previous section with the exact same content.

First, you can create an empty `secrets` bucket named `my-secrets` as follows:
...
Then, you will have to populate that `secrets` bucket by editing its contents
with the help of the next command:

The previous command allows you to edit the contents of `my-secrets` using
your favorite text editor. You will have to add the secrets name and value
pairs you want. Later on in this document you will learn more ways of editing
the contents of existing `secrets` buckets.

Although you can create `secrets` buckets any way you want, there is only one
way you can used the contents of a `secrets` bucket, which will be described in
the next section.
...
Alternatively, you can edit the contents of a `secrets` bucket directly:
...
you from having to remember the YAML file that created the `secrets` bucket in
case you want to keep it somewhere, send it to another Semaphore 2.0 user or
use it in a different Semaphore 2.0 project.
